...
静态插件没有 `harness` / `host` 全局，Host↔Client 走 webServer HTTP 路由。fnOS 门户网关的路径改写表（`bridge.mjs` 的 `ROOT_PATH_PREFIXES`）已包含 `/_dsh/`，因此该前缀下的路由在直连 `127.0.0.1:23006` 与门户 `/app/dsh-qddev` 下都可达，**不需要再改网关配置**。
